...
test.describe('transaction inspector tests based on mock data with mixed authorities', () => {
  ...
  test('Testcase 3.2', async ({ analyzeAndCompareTransaction, mockData }) => {
    const isMatching = await analyzeAndCompareTransaction(mockData.ownerAuthorityToSatisfyPostingTransaction as ApiTransaction, mockData.ownerAuthorityToSatisfyPostingExpectedResult as IProcessedTransaction)
