...
Run a Docker container instance running TX Inspector application.
OPTIONS:
  --image=IMAGE                         Docker image to run (Obligatory)
  --port=PORT                           Port to be exposed (Obligatory)
  --name=NAME                           Container name to be used (default: tx-inspector)
...
PORT=${PORT:-""}
CONTAINER_NAME=${CONTAINER_NAME:-"tx-inspector"}
DETACH=${DETACH:-false}
